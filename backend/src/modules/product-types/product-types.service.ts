...
export class ProductTypesService {
    ...
    async create(dto: CreateProductTypeDto) {
        ...
        console.log('Mapped productType entity for saving:', JSON.stringify(productType, null, 2));
        ...
        console.log('Saved productType (first pass):', JSON.stringify(savedProductType, null, 2));
            ...
            if (attr.enum_options && attr.enum_options.length > 0) {
                for (const option of attr.enum_options) {
                    if (option.image_url && option.image_url.includes('/temp/')) {
                        const oldPublicId = this.cloudinaryService.extractPublicId(option.image_url);
                        console.log(`Processing image for option ${option.id}. Old Public ID: ${oldPublicId}`);
                        if (oldPublicId) {
                            const newPublicId = `product-types/${savedProductType.id}/${attr.id}/${option.id}`;
                            const newUrl = await this.cloudinaryService.renameImage(oldPublicId, newPublicId);
