...
export class PrintingService {
  ...
  async generateOpticalPdf(id: string, res: Response) {
    ...
    doc
      .font('VietnameseBold')
      .fontSize(5)
      .text('LOHA EYEWEAR', 7, currentY, { width: 75, align: 'center' });
    currentY = doc.y + 0;
  ...
  async testPrinterDirection(res: Response) {
    ...
    // Tên SP (Dùng font Bold nếu đã register)
    doc.font('Vietnamese').fontSize(5).text(name, 5, currentY, {
      width: 75,
      align: 'center',
      lineGap: 0.5,
    });
    ...
    // In Barcode tại vị trí y2 + 10 (để cách mã SP ra một chút)
    // Tên SP (Dùng font Bold nếu đã register)
    doc.font('Vietnamese').fontSize(5).text(name, 5, doc.y, {
      width: 75,
      align: 'center',
      lineGap: 0.5,
    });
  ...
  async testPrintFullPage(res: Response) {
    ...
    doc.fillColor('red').fontSize(10).text('GOC 0,0 - MAC DINH', 5, 5);
