...
export function ProductImages({ form }: { form: UseFormReturn<any> }) {
  ...
  // Lưu preview URLs riêng (dùng secure_url để hiển thị ảnh)
  // Form field image_url lưu public_id để backend rename được
  ...
  //       // Lưu public_id vào form (backend cần để rename)
  //       append({
  //         image_url: res.public_id,
  //         sort_order: newIndex,
  //         is_primary: false,
  //       });
  //       // Lưu secure_url riêng để preview
  //       setPreviewUrls((prev) => ({ ...prev, [newIndex]: res.secure_url }));
  ...
  const onDrop = useCallback(
    async (acceptedFiles: File[]) => {
      ...
      for (const file of acceptedFiles) {
        try {
          const res = await ProductService.uploadToCloudinary(
            file,
            "temp/products/gallery",
          );
          append({
            image_url: res.public_id, // Gửi về Backend rename
            preview_url: res.secure_url, // Hiển thị UI
    ...
    <div className="space-y-4 border p-4 rounded-lg bg-slate-50/30">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-4">
        {fields.map((field, index) => (
          <div
            key={field.id}
            className="relative aspect-square group border rounded-lg bg-white overflow-hidden shadow-sm"
          >
            {/* <img
            ...
            /> */}
            <img
              src={watch(`product_images.${index}.preview_url`)}
              className="w-full h-full object-cover"
            />
            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-between p-1">
              ...
              <Button
                type="button"
                variant={
                  watch(`product_images.${index}.is_primary`)
                    ? "default"
                    : "secondary"
                }
                ...
                onClick={() => {
                  fields.forEach((_, i) =>
                    setValue(`product_images.${i}.is_primary`, i === index),
                  );
                }}
              >
        ...
        >
          ...
          <p className="text-[10px] text-slate-500 mt-1">
            {isDragActive ? "Thả vào đây" : "Thêm ảnh"}
          </p>
        </div>
      </div>
    </div>
