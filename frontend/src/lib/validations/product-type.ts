...
export const CreateProductTypeConfigAttributeSchema = z.object({
  ...
  key: z
    ...
    .regex(
      /^[a-zA-Z0-9_]+$/,
      "Mã thuộc tính chỉ được chứa chữ cái, số và dấu gạch dưới",
    ),
...
export const CreateProductTypeSchema = z.object({
  ...
  prefix: z
    .string()
    .min(1, "Mã loại sản phẩm không được để trống")
    .regex(
      /^[A-Z0-9_]+$/,
      "Mã loại sản phẩm chỉ được chứa chữ cái in hoa, số và dấu gạch dưới",
    ),
...
export type CreateProductTypeConfigAttributeInput = z.infer<
  typeof CreateProductTypeConfigAttributeSchema
>;
export type ValidationRulesInput = z.infer<typeof ValidationRulesSchema>;
export type AttributeEnumOptionInput = z.infer<
  typeof AttributeEnumOptionSchema
>;
